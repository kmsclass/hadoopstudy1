...
//hadoop2이후 버전으로 코딩. Mapper, Reducer를 내부클래스로 코딩
public class WordCountTest2 {
	public static void main(String[] args) throws IOException {
		JobConf conf = new JobConf(WordCountTest2.class); //작업 + 환경설정
		conf.setJobName("wordcount2");
		conf.setOutputKeyClass(Text.class);
		conf.setOutputValueClass(IntWritable.class);
		conf.setMapperClass(Map.class); //맵퍼 설정
//		conf.setCombinerClass(Reduce.class); //컴바인너 설정 : 맵퍼 리듀서사이의 리듀서의 기능을 통해 성능 향상.
		conf.setReducerClass(Reduce.class); //리듀서 설정
		conf.setInputFormat(TextInputFormat.class);
		conf.setOutputFormat(TextOutputFormat.class);
		//여러개의 입력파일 설정
		FileInputFormat.setInputPaths(conf, new Path("infile/in.txt"),new Path("infile/in2.txt"));// 입력파일 설정
		//출력파일 설정
